...
Author: Tuna
Date: 2025/2/18
Description: jl_sell_price_strategy
"""
...
class JlSellPriceStrategy(models.Model):
    _name = 'jl.sell.price.strategy'
    _description = '销售价格策略'
    ...
    def button_done(self):
        ...
        strategy_id = self.env['jl.sell.price.strategy'].search([('partner_id','=',self.partner_id.id),('goods_id','=',self.goods_id.id),('state','=','done'),('active','=','True')],limit=1)
        if strategy_id:
            raise UserError("价格策略客户%s\商品%s已经存在，无需再创建" %(self.partner_id.name,self.goods_id.name))
        self.state = 'done'
