...
class JlMesOusRefund(models.Model):
    ...
    def button_done(self):
        ...
        for line in self.line_ids:
            ...
            if float(line.qty) + float(line.ous_line_id.refund_qty) >  float(line.ous_line_id.done_qty):
                raise UserError('退料数量超过了已领数量')
        for record in self.line_ids:
            record.ous_line_id.write({
                'refund_qty':float(record.ous_line_id.refund_qty) + float(record.qty)
            })
        move_id = self.env['jl.move'].create({
            'ous_id': self.plm_id.id,
            'ous_refund_id': self.id,
            'origin': self._name,
            'state': 'done',
            'line_in_ids': [(0, 0, {
    ...
    def button_draft(self):
        ...
        for line in self.line_ids:
            line.ous_line_id.write({
                'refund_qty': float(line.ous_line_id.refund_qty) - float(line.qty)
            })
        ids = self.env['jl.move'].search([('ous_refund_id','=',self.id),('ous_id','=',self.ous_id.id),('state','=','done')])
        for id in ids:
            id.unlink()
        self.write({
            'state': 'draft',
    ...
    name = fields.Char('单据编号',
                       index=True,
                       copy=False,
                       default=lambda self: self.env['ir.sequence'].next_by_code('jl.mes.ous.refund'),
    ...
    ous_id = fields.Many2one('jl.mes.ous', '委外生产工单', ondelete='cascade',help='关联委外生产工单')
    date = fields.Date('单据日期', default=lambda self: fields.Date.context_today(self), required=True)
    line_ids = fields.One2many('jl.mes.ous.refund.line', 'ous_pick_id', ondelete='cascade',
                               help='生产退料单明细行')
    note = fields.Char('备注')
...
class JlMesOusRefundLine(models.Model):
    _name = 'jl.mes.ous.refund.line'
    _description = '委外生产退料单明细行'
    ...
    ous_pick_id = fields.Many2one('jl.mes.ous.refund', '委外生产领料单', ondelete='cascade')
    ous_line_id = fields.Many2one('jl.mes.ous.line', '委外生产工单明细', ondelete='cascade')
...
class JlMoveLine(models.Model):
    ...
    ous_refund_id = fields.Many2one('jl.mes.ous.refund', '委外生产退料单', ondelete='cascade', help='绑定委外生产退料单')
