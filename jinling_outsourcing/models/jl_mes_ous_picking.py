...
class JlMesOusPicking(models.Model):
    ...
    def button_done(self):
        ...
        for record in self.line_ids:
            if float(record.qty) + float(record.ous_line_id.done_qty) >  float(record.ous_line_id.neck_qty):
                raise UserError('领料数量超过了待领数量')
            record.ous_line_id.write({
                'neck_qty':float(record.ous_line_id.neck_qty) - float(record.qty),
                'done_qty':float(record.ous_line_id.done_qty) + float(record.qty)
            })
    ...
    def button_draft(self):
        self.ensure_one()
        for line in self.line_ids:
            line.ous_line_id.write({
                'neck_qty': float(line.ous_line_id.neck_qty) + float(line.qty),
                'done_qty': float(line.ous_line_id.done_qty) - float(line.qty)
            })
        ids = self.env['jl.move'].search([('ous_pick_id','=',self.id),('ous_id','=',self.ous_id.id),('state','=','done')])
        for id in ids:
            id.unlink()
        self.write({
            'state': 'draft',
    ...
    name = fields.Char('单据编号',
                       index=True,
                       copy=False,
                       default=lambda self: self.env['ir.sequence'].next_by_code('jl.mes.ous.picking'),
    ...
    line_ids = fields.One2many('jl.mes.ous.picking.line', 'ous_pick_id', ondelete='cascade',
                               help='生产领料单明细行')
    note = fields.Char('备注')
...
class JlMesOusPickingLine(models.Model):
    _name = 'jl.mes.ous.picking.line'
    _description = '委外生产领料单明细行'
    ...
    ous_pick_id = fields.Many2one('jl.mes.ous.picking', '生产领料单', ondelete='cascade')
    ous_line_id = fields.Many2one('jl.mes.ous.line', '生产工单明细', ondelete='cascade')
    ...
    note = fields.Char('备注')
    state = fields.Selection(STATE, '确认状态', related='ous_pick_id.state')

class JlMove(models.Model):
    _inherit = 'jl.move'

    ous_pick_id = fields.Many2one('jl.mes.ous.picking', '委外生产领料单', ondelete='cascade', help='绑定委外生产领料单')
