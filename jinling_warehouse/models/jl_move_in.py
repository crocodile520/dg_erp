...
class JlMoveIn(models.Model):
    ...
    def button_done(self):
        ...
        move_ids = self.env['jl.move'].search(
            [('move_in_id', '=', self.id), ('state', '=', 'done')])
        if move_ids:
            move_ids.state = 'done'
            for move_id in move_ids:
                move_id.line_in_ids.state = 'done'
        else:
    ...
    def button_draft(self):
        ...
        move_ids = self.env['jl.move'].search(
            [('move_in_id', '=', self.id), ('state', '=', 'done')])
        for move_id in move_ids:
            for line in move_id.line_in_ids:
                line.state = 'draft'
            move_id.state = 'draft'
        self.write({
            "state": "draft",
    ...
    note = fields.Char('备注')
    line_ids = fields.One2many('jl.move.in.line', 'move_in_id', '额外入库单明细行', ondelete='cascade', help='关联移库单')
...
class JlMoveInLine(models.Model):
    _name = 'jl.move.in.line'
    _description = '额外入库单明细行'

    move_in_id = fields.Many2one('jl.move.in', '额外入库单', ondelete='cascade')
...
class JlMove(models.Model):
    ...
    move_in_id = fields.Many2one('jl.move.in', '额外入库单', ondelete='cascade', help='绑定额外入库单')
