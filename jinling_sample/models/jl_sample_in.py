...
class JlSampleIn(models.Model):
    ...
    def button_done(self):
        ...
        if float(self.qty) + float(self.order_id.in_qty) > float(self.order_id.qty):
            raise UserError("入库数量不可以大于样品订单数量")
        self.order_id.write({
            'in_qty':self.order_id.in_qty + self.qty
        })
        move_id = self.env['jl.move'].create({
            'sample_order_id': self.order_id.id,
            'sample_in_id': self.id,
            'origin': self._name,
            'state':'done',
            'line_in_ids':[(0,0,{
        ...
        if self.order_id.qty - self.order_id.in_qty > 0:
            self.env['jl.sample.in'].create({
                'order_id': self.order_id.id,
                'goods_id': self.goods_id.id,
                'qty': self.order_id.qty - self.order_id.in_qty,
                'warehouse_id': self.warehouse_id.id
            })
        self.write({
            'state':'done',
    ...
    def button_draft(self):
        self.ensure_one()
        self.order_id.write({
            'in_qty':float(self.order_id.in_qty) - float(self.qty)
        })
        ids = self.env['jl.sample.in'].search([('order_id','=',self.order_id.id),('state','=','draft')])
        for id in ids:
            id.unlink()
        move_ids = self.env['jl.move'].search([('sample_order_id','=',self.order_id.id),('sample_in_id','=',self.id),('state','=','done')])
    ...
    name = fields.Char('单据编号',
                       index=True,
                       copy=False,
                       default=lambda self: self.env['ir.sequence'].next_by_code('jl.sample.in'),
    ...
    order_id = fields.Many2one('jl.sample.order', '样品订单', ondelete='cascade', help='绑定样品订单')
    ...
    line_ids = fields.One2many('jl.move.line', 'sample_in_id', '移库单明细', ondelete='cascade', help='关联移库单')
...
class JlMoveLine(models.Model):
    ...
    sample_in_id = fields.Many2one('jl.sample.in','样品入库单',ondelete='cascade',help='绑定样品入库单')
