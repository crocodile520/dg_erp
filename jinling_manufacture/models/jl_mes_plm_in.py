...
class JlMesPlmIn(models.Model):
    ...
    def button_done(self):
        ...
        self.plm_id.write({
            'buy_qty':self.plm_id.buy_qty + self.qty
        })
    ...
    def button_draft(self):
        ...
        self.plm_id.write({
            'buy_qty':float(self.plm_id.buy_qty) - float(self.qty)
        })
        ids = self.env['jl.mes.plm.in'].search([('plm_id','=',self.plm_id.id),('state','=','draft')])
        for id in ids:
            id.unlink()
        move_ids = self.env['jl.move'].search([('plm_id','=',self.plm_id.id),('plm_in_id','=',self.id),('state','=','done')])
    ...
    name = fields.Char('单据编号',
                       index=True,
                       copy=False,
                       default=lambda self: self.env['ir.sequence'].next_by_code('jl.mes.plm.in'),
    ...
    quality_id = fields.Many2one('jl.quality', '质检单', ondelete='cascade', help='绑定质检单')
    ...
    line_ids = fields.One2many('jl.move.line', 'plm_in_id', '移库单明细', ondelete='cascade', help='关联移库单')
...
class JlMoveLine(models.Model):
    ...
    plm_in_id = fields.Many2one('jl.mes.plm.in','生产入库单',ondelete='cascade',help='绑定生产入库单')