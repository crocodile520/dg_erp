...
class JlMesPlmPicking(models.Model):
    ...
    def button_done(self):
        ...
        for record in self.line_ids:
            ...
            record.plm_line_id.write({
                'neck_qty':float(record.plm_line_id.neck_qty) - float(record.qty),
                'done_qty':float(record.plm_line_id.done_qty) + float(record.qty)
            })
    ...
    def button_draft(self):
        ...
        for line in self.line_ids:
            line.plm_line_id.write({
                'neck_qty': float(line.plm_line_id.neck_qty) + float(line.qty),
                'done_qty': float(line.plm_line_id.done_qty) - float(line.qty)
            })
    ...
    name = fields.Char('单据编号',
                       index=True,
                       copy=False,
                       default=lambda self: self.env['ir.sequence'].next_by_code('jl.mes.plm.picking'),
    ...
    line_ids = fields.One2many('jl.mes.plm.picking.line', 'pick_id', ondelete='cascade',
                               help='生产领料单明细行')
    note = fields.Char('备注')
