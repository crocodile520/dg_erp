...
class JlBuyOrder(models.Model):
    ...
    def action_buy_warehousing_view(self):
        ...
        if self.buy_warehousing_count > 1:
            ...
            warehousing_id = self.env['jl.buy.warehousing'].search(
                [('order_id', '=', self.id), ('is_return', '=', False)])
    ...
    def _compute_buy_warehousing_count(selfs):
        for self in selfs:
            self.buy_warehousing_count = self.env['jl.buy.warehousing'].search_count(
                [('order_id', '=', self.id), ('is_return', '=', False)])
    ...
    user_id = fields.Many2one('hr.employee', '采购员', default=lambda self: self.env.user.employee_id.id,
                              ondelete='cascade', requierd=True,
                              )
    apply_user_id = fields.Many2one('hr.employee', '采购申请人', default=lambda self: self.env.user.employee_id.id,
                                    ondelete='cascade', requierd=True,
                                    )
    ...
    line_ids = fields.One2many('jl.buy.order.line', 'order_id', '采购订单明细行', ondelete='cascade',
                               help='关联采购订单明细行')
